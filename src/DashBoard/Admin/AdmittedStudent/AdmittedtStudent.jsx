...
const AdmittedStudent = () => {
  ...
  const handleStatusUpdate = async (admissionId, newStatus) => {
    try {
      const response = await fetch(`http://localhost:5000/admissions/${admissionId}`, {
        method: 'PATCH',
        headers: {
  ...
  const handleUpdateStudent = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`http://localhost:5000/admissions/${editFormData.admissionId}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <div className="p-6">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-800 mb-2">Admitted Students</h1>
        <p className="text-gray-600">Manage and monitor all student admissions</p>
      </div>
                ...
                <tr key={student.admissionId} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div>
                      ...
                      <div className="text-xs text-gray-700 font-semibold">ID: {student.studentID || student.admissionId}</div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex gap-2">
                      ...
                      {student.status === 'Pending' && (
                        <>
                          <button
                            onClick={() => handleStatusUpdate(student.admissionId, 'approved')}
                            className="text-green-600 hover:text-green-900"
                            title="Approve"
                          >
                            <FaCheck />
                          ...
                          <button
                            onClick={() => handleStatusUpdate(student.admissionId, 'rejected')}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    <p><strong>Class Level:</strong> {selectedStudent.classLevel}</p>
                    <p><strong>Version:</strong> {selectedStudent.version}</p>
                    <p><strong>Institution:</strong> {selectedStudent.currentInstitution}</p>
                    <p><strong>Fee:</strong> ৳{selectedStudent.courseFee}</p>
                    <p><strong>Status:</strong> 
                      <span className={`ml-2 px-2 py-1 text-xs rounded-full ${getStatusBadge(selectedStudent.status)}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setEditFormData({...editFormData, courseFee: e.target.value})}
                    />
                  </div>
                </div>
