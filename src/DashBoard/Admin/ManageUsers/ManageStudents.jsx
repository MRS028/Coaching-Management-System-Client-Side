...
const ManageStudents = () => {
  ...
  const filteredStudents = sortedUsers
    .filter((user) => user.role === "student" || user.role === " ") // শুধুমাত্র student ফিল্টার
    .filter(
      (user) =>
        `${user?.name} ${user?.email}`
          .toLowerCase()
          .includes(searchTerm.toLowerCase()) // সার্চ ফিল্টার
    );
  ...
  return (
    <div>
      <SectionTitle title={"Manage Students"} subtitle={"Dreams come true"} />

      {/* Search Box */}
      <div className="lg:mx-5 flex flex-col items-start relative">
        <label className="mb-1 font-semibold">Search a Student</label>
        <input
          type="text"
      ...
      <div className="overflow-x-auto w-full p-4 bg-white shadow-lg rounded-lg">
        <table className="table w-full border border-gray-300">
          <thead className="border-r border-gray-300">
            <tr className="bg-amber-400 text-xl text-gray-800 border-r border-gray-300">
