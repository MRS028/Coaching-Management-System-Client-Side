...
const CentralRoutine = () => {
  ...
  const { routines, loading, refetch, updateRoutine, isUpdating } = useRoutine();

  // Get the schedule for the selected class and version
  const routineData =
    routines.find((r) => r.class === selectedClass)?.subjects.find((s) => s.name === selectedVersion)?.schedule || [];
  ...
  const downloadPDF = () => {
    ...
    autoTable(doc, {
        ...
        body: tableRows,
        startY: 30,
        styles: { fontSize: 11, cellPadding: 3, lineWidth: 0.2, lineColor: [0, 0, 0] },
    ...
    const textWidth1 = doc.getStringUnitWidth(footerText1) * doc.internal.getFontSize() / doc.internal.scaleFactor;
    const textWidth2 = doc.getStringUnitWidth(footerText2) * doc.internal.getFontSize() / doc.internal.scaleFactor;
    ...
    doc.setFontSize(12);
    doc.text(footerText1, doc.internal.pageSize.width - textWidth1 - 18, footerY);
    doc.text(footerText2, doc.internal.pageSize.width - textWidth2 - 14, footerY + 5 );
            ...
            <div className="bg-white rounded-lg shadow-lg w-full max-w-lg sm:w-11/12 max-h-[90vh] overflow-y-auto md:w-2/3 lg:w-1/2 xl:w-1/3">
              <div className="p-4 sm:p-6">
                ...
                <div className="space-y-4">
                  {['time', 'Saturday', 'Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday'].map((day) => (
                    <div key={day}>
